...
- Given ada perubahan status, Then notifikasi dikirim
🟢 US-08 – Review Barang
User Story
...
User Story
As an admin, I want memantau aktivitas platform seperti jumlah user, owner, barang, dan transaksi, so that sistem berjalan dengan baik
Acceptance Criteria
- Given admin login, When membuka dashboard, Then tampil jumlah pemilik barang, jumlah penyewa, total barang aktif di   katalog, dan total saldo pemilik barang.
...
User Story
As an admin, I want membekukan akun user yang memiliki review buruk atau laporan negatif, so that saya dapat menjaga kualitas dan keamanan platform
Acceptance Criteria
- Given terdapat review buruk atau laporan dari user lain, When admin melakukan pengecekan, Then admin dapat            membekukan akun user tersebut
- Given akun telah dibekukan, When user mencoba login, Then akses ditolak oleh sistem
- Given admin ingin mengaktifkan kembali akun, When admin mengubah status akun, Then user dapat menggunakan sistem      kembali
