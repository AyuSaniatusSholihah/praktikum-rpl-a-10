<x-admin-layout title="Dashboard - SEWAIN Admin" headerTitle="Dashboard" pageId="page-dashboard">
    ...
    <div class="stat-cards">
        <x-admin-stat-card label="Total User" value="{{ $stats['total_user'] }}" changeText="Total user terdaftar"
            ...
            <svg width="32" height="24" viewBox="0 0 32 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path opacity="0.587821"
                    d="M24.001 6.66699C26.2101 6.66699 28.001 8.45785 28.001 10.667C28.0008 12.876 26.21 14.667 24.001 14.667C21.7921 14.6668 20.0012 12.8759 20.001 10.667C20.001 8.45796 21.792 6.66717 24.001 6.66699ZM12.001 0C14.9464 0 17.3348 2.38764 17.335 5.33301C17.335 8.27853 14.9465 10.667 12.001 10.667C9.05561 10.6668 6.66797 8.27842 6.66797 5.33301C6.66814 2.38775 9.05572 0.000175999 12.001 0Z"
                    fill="#2485A8" />
                <path fill-rule="evenodd" clip-rule="evenodd"
                    d="M11.9774 13.3335C18.3611 13.3335 23.6061 16.391 23.997 22.9331C24.0125 23.1937 23.9966 24.0005 22.995 24.0005H0.969619C0.635144 24.0001 -0.0272745 23.2787 0.000869049 22.9321C0.517816 16.5688 5.68241 13.3336 11.9774 13.3335ZM23.4686 16.0024C28.0103 16.0523 31.7187 18.3471 31.9979 23.1997C32.0092 23.3952 31.9977 24.0005 31.2743 24.0005H26.1337C26.1337 20.9998 25.1416 18.2305 23.4686 16.0024Z"
                    fill="#2485A8" />
            </svg>
        </x-admin-stat-card>

        <x-admin-stat-card label="Total Owner" value="{{ $stats['total_owner'] }}" changeText="User yang punya katalog"
        ...
        <x-admin-stat-card label="Total Rent" value="{{ $stats['total_rent'] }}" changeText="Total transaksi penyewaan"
        ...
        <div class="wallet-card">
            <div class="wc-icon">
                <svg width="35" height="35" viewBox="0 0 35 35" fill="none" xmlns="http://www.w3.org/2000/svg"
                    ...
                    <defs>
                        <pattern id="pattern0_3428_8106" patternContentUnits="objectBoundingBox" width="1"
                            height="1">
                            <use xlink:href="#image0_3428_8106" transform="scale(0.01)" />
                        </pattern>
                        <image id="image0_3428_8106" width="100" height="100" preserveAspectRatio="none"
        ...
        <div class="wallet-card">
            <div class="wc-icon">
                <svg width="38" height="38" viewBox="0 0 38 38" fill="none"
                    ...
                    <!-- Kotak kiri atas -->
                    <rect x="2" y="2" width="13" height="13" rx="1.5" stroke="white"
                        stroke-width="1.8" fill="none" />
                    <rect x="5" y="5" width="7" height="7" rx="0.5" fill="white" />
                    <!-- Kotak kanan atas -->
                    <rect x="23" y="2" width="13" height="13" rx="1.5" stroke="white"
                        stroke-width="1.8" fill="none" />
                    <rect x="26" y="5" width="7" height="7" rx="0.5" fill="white" />
                    <!-- Kotak kiri bawah -->
                    <rect x="2" y="23" width="13" height="13" rx="1.5" stroke="white"
                        stroke-width="1.8" fill="none" />
            ...
            <div class="user-card">
                ...
                @endif
                    ...
                    <button class="btn-view-more"
                        onclick="location.href='{{ route('admin.users.detail', $u->id) }}'">View More</button>
                </div>
            </div>
        @empty
            ...
            <tbody>
                @forelse($itemsPreview as $item)
                    <tr>
                        ...
                        <td>
                            <div class="item-info"><img class="item-thumb"
                                    src="{{ $item->foto_barang ? asset('storage/' . $item->foto_barang) : 'https://placehold.co/60x60?text=No+Image' }}"
                                    alt=""
                                    onerror="this.src='https://placehold.co/60x60?text=No+Image'">{{ $item->nama_barang }}
                            </div>
                        </td>
                        ...
                        <td><button class="btn-view"
                                onclick="location.href='{{ route('admin.items.detail', $item->id) }}'">View
                                Details</button></td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="7" style="text-align:center; color:#727272; padding:16px;">Belum ada item.
                        </td>
                    </tr>
                @endforelse
            </tbody>
            ...
            <tbody>
                @forelse($transaksiPreview as $trans)
                    <tr>
                        ...
                        <td>
                            <div class="item-info"><img class="item-thumb"
                                    src="{{ optional($trans->barang)->foto_barang ? asset('storage/' . $trans->barang->foto_barang) : 'https://placehold.co/60x60?text=No+Image' }}"
                                    alt=""
                                    onerror="this.src='https://placehold.co/60x60?text=No+Image'">{{ $trans->barang->nama_barang ?? '-' }}
                            </div>
                        </td>
                        <td>{{ optional($trans->created_at)->translatedFormat('d M Y, H.i') }}</td>
                        <td><span class="badge {{ $trans->statusBadgeClass() }}">{{ $trans->statusLabel() }}</span>
                        </td>
                        <td><button class="btn-view"
                                onclick="location.href='{{ route('admin.transactions.detail', $trans->id) }}'">View
                                Details</button></td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="8" style="text-align:center; color:#727272; padding:16px;">Belum ada
                            transaksi.</td>
                    </tr>
                @endforelse
            </tbody>
