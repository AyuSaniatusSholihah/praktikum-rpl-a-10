<x-layout title="Katalog Produk — SEWAIN">
    ...
    @php
        $catalogItems = isset($barangs) ? $barangs : (isset($barang) ? collect([$barang]) : collect());
    @endphp
    ...
    <section class="katalog-page">
        <div class="container" style="max-width:1280px;">
            ...
            @if ($catalogItems->isEmpty())
                <div class="katalog-empty-state"
                    style="padding:48px 24px; text-align:center; color:#6b7280; font-size:16px;">
                    Katalog belum tersedia. Silakan hubungi pemilik toko.
                </div>
            @else
                <div class="katalog-grid"
                    ...
                    @foreach ($catalogItems as $item)
                        <article class="katalog-card"
                            style="background:#fff; border:1px solid #e5e7eb; border-radius:20px; overflow:hidden; box-shadow:0 10px 25px rgba(15,23,42,.06);">
                            <div style="aspect-ratio:4/3; background:#f3f4f6; overflow:hidden;">
                                <img src="{{ $item->foto_barang ? asset('storage/katalog_images/' . $item->foto_barang) : 'https://placehold.co/600x450?text=No+Image' }}"
                            ...
                            <div style="padding:18px; display:grid; gap:10px;">
                                <div>
                                    <h2 style="margin:0; font-size:18px; line-height:1.3; color:#111827;">
                                        {{ $item->nama_barang }}</h2>
                                    <div style="margin-top:6px; font-size:18px; font-weight:700; color:#0f766e;">
                                        Rp
                                        {{ number_format((float) ($item->harga_sewa ?? ($item->harga ?? 0)), 0, ',', '.') }}
                                    </div>
                                </div>

                                <p style="margin:0; color:#4b5563; font-size:14px; line-height:1.6;">
                                    {{ \Illuminate\Support\Str::limit($item->deskripsi ?? 'Deskripsi produk belum tersedia.', 120) }}
                                </p>

                                @if (isset($item->stok))
                                    <div
                                        style="font-size:13px; font-weight:600; color:{{ (int) $item->stok > 0 ? '#166534' : '#b91c1c' }};">
                                        Stok: {{ (int) $item->stok > 0 ? 'Tersedia' : 'Habis' }}
                                    </div>
                                @endif

                                @if (!empty($item->tanggal_item_mulai) || !empty($item->tanggal_item_tidak_tersedia))
                                    <div style="display:grid; gap:4px; font-size:13px; color:#6b7280;">
                                        @if (!empty($item->tanggal_item_mulai))
                                            <span>Mulai tersedia:
                                                {{ \Carbon\Carbon::parse($item->tanggal_item_mulai)->translatedFormat('d M Y') }}</span>
                                        @endif
                                        @if (!empty($item->tanggal_item_tidak_tersedia))
                                            <span>Tidak tersedia mulai:
                                                {{ \Carbon\Carbon::parse($item->tanggal_item_tidak_tersedia)->translatedFormat('d M Y') }}</span>
                                        @endif
                                    </div>
                                @endif

                                @if (isset($item->kategori))
                                    <div style="font-size:13px; color:#6b7280;">Kategori:
                                        {{ $item->kategori->nama_kategori ?? $item->kategori }}</div>
                                @endif
                            </div>
                        </article>
                    @endforeach
            ...
            <div class="product-detail-col">
                ...
                <div class="viewing-now">
                    <svg width="20" height="13" viewBox="0 0 20 13" fill="none"
                        ...
                        <path
                            d="M19.7693 5.56008C17.8115 2.23779 14.1601 0.017334 10 0.017334C5.83821 0.017334 2.18766 2.23935 0.23078 5.56008C0.0796862 5.81647 0 6.10864 0 6.40624C0 6.70384 0.0796862 6.99601 0.23078 7.2524C2.18859 10.5747 5.83995 12.7951 10 12.7951C14.1618 12.7951 17.8124 10.5731 19.7693 7.25237C19.9203 6.99598 20 6.70382 20 6.40622C20 6.10863 19.9203 5.81646 19.7693 5.56008ZM10 11.1284C6.43904 11.1284 3.33019 9.22911 1.66668 6.40622C3.19991 3.80438 5.96102 1.98713 9.17231 1.71848C9.51245 2.06865 9.72224 2.54615 9.72224 3.07289C9.72224 4.14678 8.85168 5.01733 7.77779 5.01733C6.70391 5.01733 5.83335 4.14678 5.83335 3.07289L5.83338 3.07143C5.47898 3.7341 5.27779 4.49108 5.27779 5.29511C5.27779 7.90313 7.392 10.0173 10 10.0173C12.608 10.0173 14.7222 7.90313 14.7222 5.29511C14.7222 4.21765 14.3611 3.22466 13.7537 2.43001C15.6842 3.23393 17.292 4.63907 18.3334 6.40622C16.6699 9.22911 13.561 11.1284 10 11.1284Z"
                ...
                <div class="qty-section">
                    ...
                    <div class="qty-controls">
                        ...
                        <a href="{{ route('checkout', ['id' => $product->id]) }}" class="btn-add-cart">Add to
                            cart</a>
                        <a href="{{ route('checkout', ['id' => $product->id]) }}" class="btn-rent-now">Rent Now!</a>
                    </div>
                </div>
                ...
                <div class="info-list">
                    <div class="info-item">
                        <svg width="14" height="14" viewBox="0 0 20 20" fill="none"
                            xmlns="http://www.w3.org/2000/svg" style="flex-shrink:0; stroke:none;">
                            <path
                                d="M15 3.33317H14.1667V2.49984C14.1667 2.27882 14.0789 2.06686 13.9226 1.91058C13.7663 1.7543 13.5543 1.6665 13.3333 1.6665C13.1123 1.6665 12.9004 1.7543 12.7441 1.91058C12.5878 2.06686 12.5 2.27882 12.5 2.49984V3.33317H7.5V2.49984C7.5 2.27882 7.4122 2.06686 7.25592 1.91058C7.09964 1.7543 6.88768 1.6665 6.66667 1.6665C6.44565 1.6665 6.23369 1.7543 6.07741 1.91058C5.92113 2.06686 5.83333 2.27882 5.83333 2.49984V3.33317H5C4.33696 3.33317 3.70107 3.59656 3.23223 4.0654C2.76339 4.53424 2.5 5.17013 2.5 5.83317V15.8332C2.5 16.4962 2.76339 17.1321 3.23223 17.6009C3.70107 18.0698 4.33696 18.3332 5 18.3332H15C15.663 18.3332 16.2989 18.0698 16.7678 17.6009C17.2366 17.1321 17.5 16.4962 17.5 15.8332V5.83317C17.5 5.17013 17.2366 4.53424 16.7678 4.0654C16.2989 3.59656 15.663 3.33317 15 3.33317ZM6.66667 14.1665C6.50185 14.1665 6.34073 14.1176 6.20369 14.0261C6.06665 13.9345 5.95984 13.8043 5.89677 13.6521C5.83369 13.4998 5.81719 13.3322 5.84935 13.1706C5.8815 13.0089 5.96087 12.8605 6.07741 12.7439C6.19395 12.6274 6.34244 12.548 6.50409 12.5159C6.66574 12.4837 6.8333 12.5002 6.98557 12.5633C7.13784 12.6263 7.26799 12.7332 7.35956 12.8702C7.45113 13.0072 7.5 13.1684 7.5 13.3332C7.5 13.5542 7.4122 13.7661 7.25592 13.9224C7.09964 14.0787 6.88768 14.1665 6.66667 14.1665ZM13.3333 14.1665H10C9.77899 14.1665 9.56702 14.0787 9.41074 13.9224C9.25446 13.7661 9.16667 13.5542 9.16667 13.3332C9.16667 13.1122 9.25446 12.9002 9.41074 12.7439C9.56702 12.5876 9.77899 12.4998 10 12.4998H13.3333C13.5543 12.4998 13.7663 12.5876 13.9226 12.7439C14.0789 12.9002 14.1667 13.1122 14.1667 13.3332C14.1667 13.5542 14.0789 13.7661 13.9226 13.9224C13.7663 14.0787 13.5543 14.1665 13.3333 14.1665ZM15.8333 9.1665H4.16667V5.83317C4.16667 5.61216 4.25446 5.4002 4.41074 5.24392C4.56702 5.08764 4.77899 4.99984 5 4.99984H5.83333V5.83317C5.83333 6.05418 5.92113 6.26615 6.07741 6.42243C6.23369 6.57871 6.44565 6.6665 6.66667 6.6665C6.88768 6.6665 7.09964 6.57871 7.25592 6.42243C7.4122 6.26615 7.5 6.05418 7.5 5.83317V4.99984H12.5V5.83317C12.5 6.05418 12.5878 6.26615 12.7441 6.42243C12.9004 6.57871 13.1123 6.6665 13.3333 6.6665C13.5543 6.6665 13.7663 6.57871 13.9226 6.42243C14.0789 6.26615 14.1667 6.05418 14.1667 5.83317V4.99984H15C15.221 4.99984 15.433 5.08764 15.5893 5.24392C15.7455 5.4002 15.8333 5.61216 15.8333 5.83317V9.1665Z"
                                fill="#181A18" />
                        </svg>
                ...
                <div class="payment-box">
                    <div class="payment-icons">
                        <div class="pay-wrap"><img src="https://img.icons8.com/color/48/visa.png"
                                style="height: 24px;width:auto;object-fit:contain;" alt="Visa" /></div>
                        <div class="pay-wrap"><img src="https://img.icons8.com/color/48/mastercard.png"
                                style="height:28px;width:auto;object-fit:contain;" alt="Mastercard" /></div>
                        <div class="pay-wrap"><img src="https://img.icons8.com/color/48/amex.png"
                                style="height:28px;width:auto;object-fit:contain;" alt="Amex" /></div>
                        <div class="pay-wrap"><img src="https://img.icons8.com/color/48/jcb.png"
                                style="height:28px;width:auto;object-fit:contain;" alt="JCB" /></div>
                        <div class="pay-wrap"><img src="https://img.icons8.com/color/48/discover.png"
                                style="height:28px;width:auto;object-fit:contain;" alt="Discover" /></div>
                        <div class="pay-wrap"><img src="https://img.icons8.com/color/48/paypal.png"
                                style="height:28px;width:auto;object-fit:contain;" alt="PayPal" /></div>
                        <div class="pay-wrap"><img src="https://img.icons8.com/color/48/bank-card-front-side.png"
                                style="height:28px;width:auto;object-fit:contain;" alt="Card" /></div>
                        <!-- Bank Indonesia pakai badge teks -->
                        <div class="pay-wrap"><img src="{{ asset('assets/img/bca.png') }}"
                                style="height:28px;width:auto;object-fit:contain;" alt="BCA" /></div>
                        <div class="pay-wrap"><img src="{{ asset('assets/img/bni.png') }}"
                                style="height:28px;width:auto;object-fit:contain;" alt="BNI" /></div>
                        <div class="pay-wrap"><img src="{{ asset('assets/img/mandiri.png') }}"
                                style="height:28px;width:auto;object-fit:contain;" alt="Mandiri" /></div>
                        <div class="pay-wrap"><img src="{{ asset('assets/img/bri.png') }}"
                                style="height:28px;width:auto;object-fit:contain;" alt="BRI" /></div>
                        <div class="pay-wrap"><img src="{{ asset('assets/img/bsi.png') }}"
                                style="height:28px;width:auto;object-fit:contain;" alt="BSI" /></div>
                        <!-- E-wallet -->
                        <div class="pay-wrap"><img src="{{ asset('assets/img/gopay.png') }}"
                                style="height:28px;width:auto;object-fit:contain;" alt="GoPay" /></div>
                        <div class="pay-wrap"><img src="{{ asset('assets/img/ovo.png') }}"
                                style="height:28px;width:auto;object-fit:contain;" alt="OVO" /></div>
                        <div class="pay-wrap"><img src="{{ asset('assets/img/dana.png') }}"
                                style="height:28px;width:auto;object-fit:contain;" alt="DANA" /></div>
                        <div class="pay-wrap"><img src="{{ asset('assets/img/spay.png') }}"
                                style="height:28px;width:auto;object-fit:contain;" alt="ShopeePay" /></div>
                    </div>
...
<aside class="minicart-sidebar" id="minicartSidebar" role="dialog" aria-label="Shopping Cart">
    ...
    <div class="minicart-footer">
        ...
        <button class="minicart-checkout"
            onclick="
          const raw = localStorage.getItem('checkoutCart');
          if (raw) {
            const items = JSON.parse(raw);
            if (items.length > 0) {
              const id = items[0].id || items[0].barang_id || items[0].product_id;
...
<x-slot:scripts>
    <script>
        /* ============ DATA PRODUK (nanti dari database) ============ */
        // Ambil data dari localStorage (fallback ke hardcode kalau kosong)
        ...
        const productInfo = {
            spesifikasi: saved.spesifikasi?.length ? saved.spesifikasi : [{
                ...
                {
                    label: 'Material',
                    nilai: '210D Oxford Waterproof PU 4500MM'
                },
                {
                    label: 'Ukuran',
                    nilai: '380 x 260 x 185 cm'
                },
            ],
            pemilik: {
        ...
        // Set display tanggal default sesuai productData (format id-ID, bukan hardcode HTML)
        document.getElementById('displayStart').textContent =
            new Date(productData.tglMulai).toLocaleDateString('id-ID', {
                day: 'numeric',
                month: 'long',
                year: 'numeric'
            });
        document.getElementById('displayEnd').textContent =
            new Date(productData.tglSelesai).toLocaleDateString('id-ID', {
                day: 'numeric',
                month: 'long',
                year: 'numeric'
            });
        ...
        /* ============ STOCK ============ */
        document.querySelector('.stock-text').innerHTML =
        `Only <strong>${productData.stok}</strong> item(s) left in stock!`;
        ...
        /* ============ AVAILABLE DATES ============ */
        const tglMulaiDisplay = new Date(productData.tglMulai).toLocaleDateString('id-ID', {
            day: 'numeric',
            month: 'long',
            year: 'numeric'
        });
        const tglSelesaiDisplay = new Date(productData.tglSelesai).toLocaleDateString('id-ID', {
            day: 'numeric',
            month: 'long',
            year: 'numeric'
        });
        document.querySelector('.info-item span').innerHTML =
            `<strong>Available Dates:</strong> ${tglMulaiDisplay} - ${tglSelesaiDisplay}`;
        ...
        const reviews = [{
                ...
                bintang: 5,
                isi: 'Tenda kualitas premium, set up gampang banget! Sangat direkomendasikan untuk camping keluarga.'
            },
            {
                nama: 'Sari M.',
                ...
                bintang: 4,
                isi: 'Cukup luas untuk 4 orang, tahan hujan. Pemilik juga responsif dan ramah.'
            },
            {
                nama: 'Reza P.',
                ...
                bintang: 5,
                isi: 'Worth it dengan harga sewanya. Highly recommended untuk kalian yang mau camping bareng keluarga!'
            },
        ];


        // Hitung rata-rata bintang otomatis
    ...
    `).join('');
        ...
        // Event thumbnail (pakai event delegation karena di-generate dinamis)
      ...
      <div style="background:#f5f5f5;border-radius:8px;padding:18px 20px;font-family:'Poppins',sans-serif;font-size:13px;display:flex;flex-direction:column;gap:10px;">
        ...
        function checkOverflow() {
            ...
            // Sementara tampilkan full buat ngukur
            removeClamp(content);
        ...
        document.getElementById('dateStart').addEventListener('change', function() {
            ...
            // ✅ Tanggal selesai minimal = tanggal mulai + 1 hari
            ...
            // Reset tanggal selesai kalau sudah terlanjur pilih sebelum tanggal mulai
        ...
        document.querySelector('[aria-label="Ask a question"]').addEventListener('click', (e) => {
            e.preventDefault();
            const waLink =
                `https://wa.me/${productInfo.pemilik.wa}?text=${encodeURIComponent('Halo, saya ingin bertanya tentang ALLTREK Tenda Camping')}`;
        ...
        document.querySelector('[aria-label="Share"]').addEventListener('click', (e) => {
            ...
            if (navigator.share) {
                navigator.share({
                    title: 'ALLTREK Tenda Camping — SEWAIN',
        ...
        document.querySelector('.btn-add-cart').addEventListener('click', () => {
            ...
            if (existingIndex > -1) {
                cart[existingIndex].qty += item.qty; // tambah qty saja
            } else {
                cart.push(item);
            }
            ...
            setTimeout(() => {
                ...
                const badge = cartEl.querySelector('.badge');
        ...
        document.querySelector('.btn-rent-now').addEventListener('click', () => {
            ...
            const item = {
                id: 'alltrek-tentastic',
                nama: document.querySelector('.product-title').textContent.trim(), // ✅ dari judul halaman
                qty: qty,
                harga: 450000,
                img: document.querySelector('.product-main-img img').src, // ✅ dari foto yang tampil
                tglMulai: tglMulai,
                tglSelesai: tglSelesai,
            };
        ...
        const SVG_REMOVE = `<svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path fill-rule="evenodd" clip-rule="evenodd" d="M10 0C4.47727 0 0 4.47727 0 10C0 15.5227 4.47727 20 10 20C15.5227 20 20 15.5227 20 10C20 4.47727 15.5227 0 10 0ZM13.37 7.91545C13.5356 7.744 13.6272 7.51436 13.6252 7.276C13.6231 7.03764 13.5275 6.80963 13.3589 6.64107C13.1904 6.47252 12.9624 6.37691 12.724 6.37484C12.4856 6.37277 12.256 6.4644 12.0845 6.63L10 8.71455L7.91545 6.63C7.83159 6.54317 7.73128 6.47392 7.62037 6.42627C7.50946 6.37863 7.39016 6.35355 7.26946 6.3525C7.14875 6.35145 7.02904 6.37445 6.91731 6.42016C6.80559 6.46587 6.70409 6.53338 6.61873 6.61873C6.53338 6.70409 6.46587 6.80559 6.42016 6.91731C6.37445 7.02904 6.35145 7.14875 6.3525 7.26946C6.35355 7.39016 6.37863 7.50946 6.42627 7.62037C6.47392 7.73128 6.54317 7.83159 6.63 7.91545L8.71455 10L6.63 12.0845C6.54317 12.1684 6.47392 12.2687 6.42627 12.3796C6.37863 12.4905 6.35355 12.6098 6.3525 12.7305C6.35145 12.8513 6.37445 12.971 6.42016 13.0827C6.46587 13.1944 6.53338 13.2959 6.61873 13.3813C6.70409 13.4666 6.80559 13.5341 6.91731 13.5798C7.02904 13.6255 7.14875 13.6485 7.26946 13.6475C7.39016 13.6465 7.50946 13.6214 7.62037 13.5737C7.73128 13.5261 7.83159 13.4568 7.91545 13.37L10 11.2855L12.0845 13.37C12.256 13.5356 12.4856 13.6272 12.724 13.6252C12.9624 13.6231 13.1904 13.5275 13.3589 13.3589C13.5275 13.1904 13.6231 12.9624 13.6252 12.724C13.6272 12.4856 13.5356 12.256 13.37 12.0845L11.2855 10L13.37 7.91545Z" fill="#9F9F9F"/>
      ...
      `;
        ...
        // Isi default kalau localStorage kosong
        ...
        if (existingCart.length === 0) {
            saveCart([{
                    ...
                    harga: 450000,
                    qty: 1,
                    img: '{{ asset('assets/img/tenda altrek.webp') }}',
                    label: 'ALLTREK',
                    tglMulai: '',
                    tglSelesai: ''
                },
                {
                    ...
                    harga: 630000,
                    qty: 1,
                    img: '{{ asset('assets/img/kebaya cream.jpg') }}',
                    label: '',
                    tglMulai: '',
                    tglSelesai: ''
                }
        ...
        // Sync badge + render minicart dari localStorage
        renderMinicart();
    </script>
</x-slot:scripts>
</x-layout>
