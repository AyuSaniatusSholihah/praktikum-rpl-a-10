...
<nav class="dash-nav">
  ...
  <a href="my-wallet.html" class="active">
    ...
    Log Out
       ...
       <div class="dash-cards" id="ownerActive"></div>
            ...
            <div class="profile-left">
                ...
                <textarea id="accRek" rows="4">Nama Rekening: BSI Syariah
No Rekening: 763098218847
A/n: Camping Groups Bandung</textarea>
                </div>
            </div>
       ...
       <div class="txn-grid" id="walletHistory"></div>
...
const walletData = [
  {
    ...
    date: '17/05/2026',
    img: '/sewain/assets/img/katalog/tent.png',  // ← tambah /sewain/
    type: 'pemasukan'
  },
  {
    ...
    date: '14/12/2025',
    img: '/sewain/assets/img/ip 18b air.webp',  // ← tambah /sewain/
    type: 'pembayaran'
  },
];

let txnIdx = 0;
function buildTxnCard(item) {
  txnIdx++;
  const cid = 'wc' + txnIdx;
  const badgeClass = item.type === 'pemasukan' ? 'pemasukan-badge' : 'pembayaran-badge';
  const badgeText = item.type === 'pemasukan' ? 'Pemasukan Sewa' : 'Pembayaran Sewa';
  return `
  <div class="txn-card-1-wrapper">
    <div class="txn-card">
      <div class="txn-img-wrap">
        ...
        <div class="txn-foto">
          <svg width="238" height="169" viewBox="0 0 238 169" fill="none">
            ...
            <image href="${item.img}" x="0" y="0" width="238" height="169"
      ...
      <div class="txn-card-info">
        ...
        <div class="date-row">${calSvg} Tanggal Sewa: ${item.start} s.d ${item.end}</div>
        <div class="date-row">${calSvg} Tanggal Transaksi: ${item.date}</div>
      </div>
    </div>
  </div>`;
}

document.getElementById('walletHistory').innerHTML = walletData.map(buildTxnCard).join('');
...
window.addEventListener('resize', adjustTxnShape); // ← tambah ini
</script>
