                ...
                <tr>
                  <td width="110" valign="top" style="position:relative;">
                    <div style="position:relative;display:inline-block;">
                      ...
                      @php
                        $imagePath = public_path('storage/' . $trans->barang->foto_barang);
                        if ($trans->barang->foto_barang && file_exists($imagePath)) {
                            // Embed image directly into the email
                            $imgSrc = $message->embed($imagePath);
                        } else {
                            // Fallback to placeholder if file is missing
                            $imgSrc = 'https://placehold.co/90x100?text=Foto';
                        }
                      @endphp
                      <img src="{{ $imgSrc }}" alt="{{ $trans->barang->nama_barang }}" width="90" height="100" style="display:block;object-fit:cover;border-radius:4px;border:1px solid #e0e0e0;">
                    </div>
                  </td>
                ...
                <tr>
                  <td width="50%" valign="top" style="padding-right: 8px;">
                    <div style="background: #fff; padding: 12px; border-radius: 8px; border: 1px solid #e8e8e8;">
                      ...
                      <div style="font-size: 13px; font-weight: 600; color: #181A18; display: flex; align-items: center; gap: 6px;">
                        📅 <span>{{ \Carbon\Carbon::parse($trans->tanggal_sewa)->translatedFormat('d F Y') }}</span>
                      </div>
                    </div>
                  </td>
                  <td width="50%" valign="top" style="padding-left: 8px;">
                    <div style="background: #fff; padding: 12px; border-radius: 8px; border: 1px solid #e8e8e8;">
                      ...
                      <div style="font-size: 13px; font-weight: 600; color: #181A18; display: flex; align-items: center; gap: 6px;">
                        📅 <span>{{ \Carbon\Carbon::parse($trans->tanggal_kembali_rencana)->translatedFormat('d F Y') }}</span>
                      </div>
                    </div>
                  </td>
                </tr>
